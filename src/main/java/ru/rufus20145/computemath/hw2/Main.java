...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // SimpleIterations.forList(task2problems, task2derivatives);

        List<CompNewton> task21equations = new ArrayList<>();
        List<NewtonInfo> task21dots = new ArrayList<>();
        addNewtons21(task21equations);
        addDots21(task21dots);

        Newton.forList(task21equations, task21dots);

        List<CompNewton> task22equations = new ArrayList<>();
        addNewtons22(task22equations);

        System.out.println("Task 2.2");
        Newton.run(task22equations, new NewtonInfo(0, 0, 0, -0.5, 1));
    }
    ...
    private static void addDots21(List<NewtonInfo> list) {
        list.add(new NewtonInfo(0.2, 0.6, 2, 0.8, 0.5));
        list.add(new NewtonInfo(0.4, 0.8, 2, -0.3, 0.6));
        list.add(new NewtonInfo(0.3, 0.2, 3, 1, 0.5));
        list.add(new NewtonInfo(0, 0.6, 2, 0.6, 0.6));
    }
